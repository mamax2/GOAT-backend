...
// 1) Recupera annuncio
$stmt = $pdo->prepare("
  SELECT id, type, remaining_spots, expires_at
  FROM announcements
...
// 3) Determina ruolo in base al type
// richiesta -> tutor si propone
// offerta   -> studente partecipa
...
try {
    ...
    $ins = $pdo->prepare("
    INSERT INTO announcement_participants (announcement_id, user_id, role)
    VALUES (?, ?, ?)
  ");
    $ins->execute([$announcementId, $userId, $role]);
